...
const Presenter = (props: { req: TypePriceRequest; marginLeft: number; max: number }) => {
  ...
  const overlays = useRecoilValue(StateChartOverlays({ code: req.code, type: 'heikin-aski' }));
  ...
  const onSarCheckboxChange = useCheckboxChange(req.code, 'heikin-aski', 'ParabolicSAR');
  const onBollingerCheckboxChange = useCheckboxChange(req.code, 'heikin-aski', 'BollingerBands');
